...
class DashboardSchedule(models.Model):
    ...
    def generate_subject(self):
        return 'Colors: Your report ' + self.dashboard.name + ' is ready. [' + timezone.now().strftime('%d/%m/%Y') + ', ' + self.time + ']'

class DashboardScheduleOption(models.Model):
    schedule  = models.ForeignKey(DashboardSchedule)
    option    = models.IntegerField()

class DashboardScheduleVisualization(models.Model):
    visualization  = models.ForeignKey(Visualization)
